...
impl<T, const MIN: isize, const MAX: isize> AddAssign<T> for Bound_isize<MIN, MAX> where T: SqueezeTo_i64 {
	#[inline(always)]
	fn add_assign(&mut self, rhs: T) {
		self.set(i64::saturating_add(self.get().pad_to_i64(), rhs.squeeze_to_i64()).squeeze_to_isize());
	}
}
