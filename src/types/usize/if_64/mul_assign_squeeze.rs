...
impl<T, const MIN: usize, const MAX: usize> MulAssign<T> for Bound_usize<MIN, MAX> where T: SqueezeTo_i128 {
	#[inline(always)]
	fn mul_assign(&mut self, rhs: T) {
		self.set(i128::saturating_mul(self.get().pad_to_i128(), rhs.squeeze_to_i128()).squeeze_to_usize());
	}
}
