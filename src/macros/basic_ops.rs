    ...
    ($Int:ty[$N:ty]) => {
        ...
        impl std::iter::Step for $Int {
            ...
            fn forward_checked(mut start: Self, count: usize) -> Option<Self> {
                ...
                if diff != 0 { Some(start) } else { None }
            }

            fn forward_overflowing(mut start: Self, count: usize) -> (Self, bool) {
                let old_start = start;
                start += count;
            ...
            fn backward_overflowing(mut start: Self, count: usize) -> (Self, bool) {
                let old_start = start;
                start -= count;
                (start, start != old_start)
            }
...
macro_rules! impl_basic_ops_assign_self_non_generic {
    ($Int:ty[$N:ty]) => {
        impl std::ops::AddAssign<$Int> for $Int {
            #[inline(always)]
            fn add_assign(&mut self, rhs: $Int) { self.set(<$N>::saturating_add(self.get(), rhs.get())); }
        }

        impl std::ops::SubAssign<$Int> for $Int {
            #[inline(always)]
            fn sub_assign(&mut self, rhs: $Int) { self.set(<$N>::saturating_sub(self.get(), rhs.get())); }
        }

        impl std::ops::MulAssign<$Int> for $Int {
            #[inline(always)]
            fn mul_assign(&mut self, rhs: $Int) { self.set(<$N>::saturating_mul(self.get(), rhs.get())); }
        }

        impl std::ops::DivAssign<$Int> for $Int {
            #[inline(always)]
            fn div_assign(&mut self, rhs: $Int) { self.set(<$N>::saturating_div(self.get(), rhs.get())); }
        }
...
macro_rules! impl_basic_ops_self_generic {
    ($Int:ident[$N:ty]) => {
        impl<const A_MIN: $N, const A_MAX: $N, const B_MIN: $N, const B_MAX: $N> std::ops::Add<$Int<B_MIN, B_MAX>>
            for $Int<A_MIN, A_MAX>
        {
            type Output = $N;

            #[inline(always)]
            fn add(self, rhs: $Int<B_MIN, B_MAX>) -> Self::Output { <$N>::saturating_add(self.get(), rhs.get()) }
        }
        ...
        impl<const A_MIN: $N, const A_MAX: $N, const B_MIN: $N, const B_MAX: $N> std::ops::Sub<$Int<B_MIN, B_MAX>>
            for $Int<A_MIN, A_MAX>
        {
            type Output = $N;

            #[inline(always)]
            fn sub(self, rhs: $Int<B_MIN, B_MAX>) -> Self::Output { <$N>::saturating_sub(self.get(), rhs.get()) }
        }
        ...
        impl<const A_MIN: $N, const A_MAX: $N, const B_MIN: $N, const B_MAX: $N> std::ops::Mul<$Int<B_MIN, B_MAX>>
            for $Int<A_MIN, A_MAX>
        {
            type Output = $N;

            #[inline(always)]
            fn mul(self, rhs: $Int<B_MIN, B_MAX>) -> Self::Output { <$N>::saturating_mul(self.get(), rhs.get()) }
        }
        ...
        impl<const A_MIN: $N, const A_MAX: $N, const B_MIN: $N, const B_MAX: $N> std::ops::Div<$Int<B_MIN, B_MAX>>
            for $Int<A_MIN, A_MAX>
        {
            type Output = $N;

            #[inline(always)]
            fn div(self, rhs: $Int<B_MIN, B_MAX>) -> Self::Output { <$N>::saturating_div(self.get(), rhs.get()) }
        }
        ...
        impl<const A_MIN: $N, const A_MAX: $N, const B_MIN: $N, const B_MAX: $N> std::ops::Rem<$Int<B_MIN, B_MAX>>
            for $Int<A_MIN, A_MAX>
        {
            type Output = $N;

            #[inline(always)]
            fn rem(self, rhs: $Int<B_MIN, B_MAX>) -> Self::Output { <$N>::rem(self.get(), rhs.get()) }
        }
...
macro_rules! impl_basic_ops_assign_self_generic {
    ($Int:ident[$N:ty]) => {
        impl<const A_MIN: $N, const A_MAX: $N, const B_MIN: $N, const B_MAX: $N> std::ops::AddAssign<$Int<B_MIN, B_MAX>>
            for $Int<A_MIN, A_MAX>
        {
            #[inline(always)]
            fn add_assign(&mut self, rhs: $Int<B_MIN, B_MAX>) { self.set(<$N>::saturating_add(self.get(), rhs.get())); }
        }

        impl<const A_MIN: $N, const A_MAX: $N, const B_MIN: $N, const B_MAX: $N> std::ops::SubAssign<$Int<B_MIN, B_MAX>>
            for $Int<A_MIN, A_MAX>
        {
            #[inline(always)]
            fn sub_assign(&mut self, rhs: $Int<B_MIN, B_MAX>) { self.set(<$N>::saturating_sub(self.get(), rhs.get())); }
        }

        impl<const A_MIN: $N, const A_MAX: $N, const B_MIN: $N, const B_MAX: $N> std::ops::MulAssign<$Int<B_MIN, B_MAX>>
            for $Int<A_MIN, A_MAX>
        {
            #[inline(always)]
            fn mul_assign(&mut self, rhs: $Int<B_MIN, B_MAX>) { self.set(<$N>::saturating_mul(self.get(), rhs.get())); }
        }

        impl<const A_MIN: $N, const A_MAX: $N, const B_MIN: $N, const B_MAX: $N> std::ops::DivAssign<$Int<B_MIN, B_MAX>>
            for $Int<A_MIN, A_MAX>
        {
            #[inline(always)]
            fn div_assign(&mut self, rhs: $Int<B_MIN, B_MAX>) { self.set(<$N>::saturating_div(self.get(), rhs.get())); }
        }

        impl<const A_MIN: $N, const A_MAX: $N, const B_MIN: $N, const B_MAX: $N> std::ops::RemAssign<$Int<B_MIN, B_MAX>>
            for $Int<A_MIN, A_MAX>
        {
            #[inline(always)]
